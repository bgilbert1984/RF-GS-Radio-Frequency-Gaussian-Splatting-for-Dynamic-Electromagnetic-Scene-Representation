...
class GaussianSplatModel(nn.Module):
    ...
    def __init__(
        ...
        self.device = device
        # Rendering backend: 'python' (reference) or 'cuda' (optimized 3DGS adapter)
        self.backend = 'python'
        ...
        # Backend selection: 'python', 'cuda-fallback', 'cuda-3dgs', 'cuda-auto'
        self.backend = (backend or "").lower()
        self.cuda_renderer = None
        if self.backend == 'python':
            print("[GaussianSplatModel] Backend = python (pure PyTorch renderer).")
        elif self.backend in ("cuda", "cuda-auto", "cuda-fallback", "cuda-3dgs"):
            # Normalize 'cuda' -> 'cuda-auto'
            mode = "cuda-auto" if self.backend in ("cuda", "cuda-auto") else self.backend
            try:
                from rf_3dgs_backend import CUDAGaussianRenderer

                self.cuda_renderer = CUDAGaussianRenderer(device=self.device, mode=mode)
                print(f"[GaussianSplatModel] Backend = {self.backend} (mode={mode}).")
            except Exception as e:
                print(
                    f"[GaussianSplatModel] Failed to init CUDA backend ({self.backend}): {e}\n"
                    "Falling back to Python renderer."
                )
                self.backend = "python"
                self.cuda_renderer = None
        else:
            raise ValueError(f"Unknown backend '{backend}'. Use 'python', 'cuda-fallback', 'cuda-3dgs', or 'cuda-auto'.")
    ...
    def render_image(
        ...
        """
        Dispatch to chosen backend: CUDA adapter (if available) or Python reference renderer.
        """
        ...
        if getattr(self, 'backend', 'python').startswith('cuda') and self.cuda_renderer is not None:
            try:
                return self._render_image_cuda(
                    ...
                    num_depth_bits=num_depth_bits,
                    depth_premultiplier=depth_premultiplier,
                )
            except Exception as e:
                print(f"[warning] CUDA backend failed, falling back to Python renderer: {e}")

        return self._render_image_python(
            camera_position=camera_position,
            camera_matrix=camera_matrix,
            width=width,
            height=height,
    ...
    def _render_image_python(
        ...
        # --- original python renderer implementation follows ---
        # Get active parameters
    ...
    def _render_image_cuda(
        ...
        """
        Use the CUDA backend adapter to render Gaussians. The adapter is a thin wrapper
        around an optimized Gaussian splatting rasterizer (if available).
        """
        if self.cuda_renderer is None:
            raise RuntimeError("CUDA renderer not available; install the 3DGS CUDA backend and set backend='cuda' on model init")
        ...
        covariance = params['covariance']
        colors = params['colors']
        opacity = params['opacity'].squeeze(-1)

        # Convert focal length to vertical FOV (approx)
        fov_y = 2.0 * math.atan((0.5 * height) / (focal_length + 1e-9))

        # Call adapter render; adapter should return H x W x 3 RGB image
        rendered = self.cuda_renderer.render(
            positions_3d=positions,
            covariances_3d=covariance,
            colors=colors,
            opacities=opacity,
            camera_to_world=camera_matrix.to(self.device),
            width=width,
            height=height,
            fov_y=float(fov_y),
        ...
        # Ensure returned tensor is HxWx3 RGB on self.device
        if isinstance(rendered, torch.Tensor):
            rgb = rendered.to(self.device)
            if rgb.dim() == 3 and rgb.shape[2] == 3:
                depth = torch.ones(height, width, device=self.device) * far_plane
                opacity_map = torch.zeros(height, width, device=self.device)
                return {'rgb': rgb, 'depth': depth, 'opacity': opacity_map}
            else:
                raise RuntimeError("CUDA renderer returned unexpected tensor shape")
        else:
            raise RuntimeError("CUDA renderer returned non-tensor result")
